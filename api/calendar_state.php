...
switch ($_SERVER['REQUEST_METHOD']) {
    case 'GET':
        // Get calendar state for specific year/month
    ...
    case 'POST':
        ...
        // Only allow Backoffice users to update calendar state
        $currentUser = requireAuth(); // Require authentication for POST requests
        
        if ($currentUser['role'] !== 'Backoffice') {
            handleError('Only Backoffice users can update calendar state', 403);
        }
        ...
        $frozenBy = $currentUser['id']; // Use the current user's ID
