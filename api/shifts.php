...
switch ($_SERVER['REQUEST_METHOD']) {
    case 'GET':
        ...
        // Prepare the query to get shifts
        $stmt = $conn->prepare("
    ...
    case 'POST':
        ...
        // Log for debugging
        error_log("Shift update - Date: $date, Shift: $shiftType, Position: $position, UserID: $userId");
        ...
        if ($result->num_rows > 0) {
            ...
            if ($position === 1) {
                ...
                $stmt->bind_param('ssi', $userId, $note, $shiftId);
            } else if ($position === 2) {
                $stmt = $conn->prepare("
                    UPDATE shifts 
                    SET user2_id = ?, note2 = ? 
            ...
            if ($position === 1) {
                $stmt = $conn->prepare("
                    INSERT INTO shifts (date, shift_type, user1_id, note1) 
                    VALUES (?, ?, ?, ?)
                ");
                ...
                $stmt->bind_param('ssss', $date, $shiftType, $userId, $note);
            } else if ($position === 2) {
                $stmt = $conn->prepare("
                    INSERT INTO shifts (date, shift_type, user2_id, note2) 
                    VALUES (?, ?, ?, ?)
                ");
