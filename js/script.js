...
async function addUser(name) {
    try {
        ...
        console.log('Benutzer erfolgreich hinzugefügt:', newUser);
        return newUser;
    } catch (error) {
...
// Create default data if API fails (fallback)
...
function setupEventListeners() {
    ...
    document.getElementById('addUser').addEventListener('click', () => {
        ...
        if (nameInput.value.trim()) {
            addUser(nameInput.value.trim());
            nameInput.value = '';
        }
    });

     // Add event handler for mobile color customize button
...
// Function to position modal relative to a target element
function positionModal(modal, targetRect) {
    // Default position above the shift
    let top = targetRect.top - modal.offsetHeight - 10;
    const left = targetRect.left + (targetRect.width / 2) - (modal.offsetWidth / 2);
    ...
    if (left < 10) {
        adjustedLeft = 10;
    } else if (left + modal.offsetWidth > window.innerWidth - 10) {
        adjustedLeft = window.innerWidth - modal.offsetWidth - 10;
    }
    
    if (window.innerWidth > 768) {
        // Desktop: position the arrow and modal
        const arrow = modal.querySelector('.shift-detail-modal-arrow');
        if (arrow) {
            const arrowLeft = targetRect.left - adjustedLeft + (targetRect.width / 2);
...
function updateUserTable() {
    ...
    staticData.users.forEach(user => {
        ...
        // Add event handlers after the row is added to the DOM
